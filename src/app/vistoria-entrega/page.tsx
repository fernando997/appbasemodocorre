...
function VistoriaEntregaContent() {
  ...
  useEffect(() => {
    ...
    chamarBubble('vistoria-entrega', { placa: placa.trim().toUpperCase(), contrato })
      ...
      .catch((err) => {
        console.error('[vistoria-entrega] Erro na chamada:', err)
        setContratoApto(false)
      })
      .finally(() => setValidandoContrato(false))
  }, [placa, contrato])
  ...
  if (validandoContrato) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#080C1F] via-[#1B2043] to-[#2E4080] flex items-center justify-center p-4">
        <div className="flex flex-col items-center gap-4">
          <Loader2 className="w-10 h-10 animate-spin text-[#6C63FF]" />
          <p className="text-sm text-white/60">Validando contrato...</p>
        </div>
      </div>
    )
  }

  if (contratoApto === false) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#080C1F] via-[#1B2043] to-[#2E4080] flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/15 shadow-[0_8px_32px_rgba(0,0,0,0.2)] p-6 text-center max-w-sm">
          <div className="w-16 h-16 rounded-2xl bg-red-500/20 flex items-center justify-center mx-auto mb-4">
          ...
          <h2 className="text-lg font-bold text-white mb-2">Vistoria indisponivel</h2>
          <p className="text-sm text-white/60">A vistoria nao pode ser iniciada pois o contrato nao esta apto. Entre em contato com a empresa para mais informacoes.</p>
        </div>
      </div>
    )
  }
