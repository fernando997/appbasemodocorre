...
function mensagemAmigavelRegistro(raw: string): string {
  ...
  if (texto.includes('nenhuma placa registrada')) return 'Sua parte foi registrada. Estamos aguardando a base concluir a vistoria da moto que está saindo.'
  if (texto.includes('placa nova')) return 'A vistoria da moto que está voltando já havia sido registrada. Atualizamos com os seus dados.'
  if (texto.includes('placa antiga')) return 'A vistoria da moto que está saindo já foi concluída pela base. Sua parte também foi registrada.'
  return raw
}
...
function VistoriaEntregaContent() {
  ...
  async function onVideoGravado(arquivo: File) {
  ...
  async function finalizarVistoria() {
    ...
    try {
      ...
      const registro = await chamarBubble('base-registro-vistoria-retirar', {
        contrato: contratoId ?? '',
      ...
      try {
        await chamarBubble('base-vitoria-sub-retirar', body, 'json')
      } catch (err) {
        ...
        return (
          <GradientBorderCard etapaKey={etapaKey} themeColor={theme.color}>
            <div className="flex flex-col items-center gap-4">
              ...
              <GravadorVideo onGravado={onVideoGravado} label="Gravar Video" labelRegravar="Gravar novamente" variant="dark" />
            </div>
          </GradientBorderCard>
        )
